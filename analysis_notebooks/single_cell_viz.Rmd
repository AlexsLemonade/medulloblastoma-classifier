...
malignant_qc_stats_file <- here::here(plots_data_dir,
                                      "10x_malignant_cell_quality_stats.tsv")
```
...
Are the differences in quality between classified and unclassified malignant cells statistically significant?

```{r}
# Add a new column tracking whether or not something is classified to use for
# statistical test
malignant_cell_qc_df <- malignant_cell_qc_df |>
  dplyr::mutate(classification = dplyr::if_else(
    predicted_labels == "Unclassified", "Unclassified", "Classified")
  )

# Hold data frames returned by rstatix::wilcox_text in a list
stats_list <- list()

# For each model type-measure pair
for (which_model in unique(malignant_cell_qc_df$model_type)) {
  for (measure_type in unique(malignant_cell_qc_df$measure)) {
    # Perform a Mann Whitney test comparing classified and unclassified cells
    stats_list[[glue::glue("{which_model}_{measure_type}")]] <- 
      malignant_cell_qc_df |>
      dplyr::filter(model_type == which_model,
                    measure == measure_type) |> 
      rstatix::wilcox_test(value ~ classification)
  }
}

# Get back in one data frame
stats_df <- stats_list |>
  dplyr::bind_rows(.id = "model_measure") |> 
  dplyr::mutate(p_adj = p.adjust(p, method = "bonferroni")) |>
  tidyr::separate(model_measure, 
                  into = c("model_type", "measure"),
                  sep = "_") 
...
Yes, all comparisons are statistically significant.

Write to file.

```{r}
readr::write_tsv(stats_df, file = malignant_qc_stats_file)
```
