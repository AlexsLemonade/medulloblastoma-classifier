...
For the 10X dataset, we will filter out cells that don't have more than 10% of informative genes observed.
...
umap_cluster_df <- smartseq_results_df |>
  dplyr::select(dplyr::all_of(cols_to_retain)) |>
...
Make a UMAP using labels from the unweighted kTSP model.

```{r}
umap_plots <- selected_sample_umap_df |>
  # Order the predicted labels in the same order of the palette for display
  dplyr::mutate(predicted_labels = factor(predicted_labels,
                                          levels = names(subgroup_colors))) |>
  # Randomly sort to help with viz
  dplyr::sample_n(nrow(selected_sample_umap_df)) |>
  ggplot(
    aes(
      x = UMAP_1,
      y = UMAP_2,
