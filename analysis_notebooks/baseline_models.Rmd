...
Balanced accuracy is the average of a model's sensitivity and specificity, measured on a per-subgroup basis.
...
if (create_models) {
  ...
  unweighted_kTSP_RF_models_list <- unweighted_kTSP_RF_models_list |>
    purrr::map(function(x) setNames(x,
                                    dplyr::recode(names(x),
...
plot_df <- baseline_list |>
  purrr::map(function(x) return_model_metrics(single_repeat = x,
  ...
  dplyr::mutate(model_type = dplyr::recode(model_type,
...
- The reverse is true for RF models, with RF (w) outperformin RF (unw) for both Array and RNA-seq test data
...
baseline_kappa_plot <- plot_df |>
  ...
  ggplot2::geom_point(shape = 16,
                      position = ggplot2::position_jitterdodge(jitter.width = 0.1,
                                                               jitter.height = 0)) +
  ggplot2::scale_x_continuous(limits = c(0.7,1)) +
...
baseline_accuracy_plot <- plot_df |>
  ...
  ggplot2::facet_wrap(~ subgroup,
                      ncol = 1) +
  ggplot2::scale_x_continuous(limits = c(0.6,1)) +
...
official_model <- which(unlist(purrr::map(baseline_list,
                                          function(x) x$official_model)))
...
if (any(train_test_samples_list$test != row.names(official_ktsp_test_results))) {
  ...
  binary_TSP_plot <- function() {multiclassPairs::plot_binary_TSP(Data = test_data_object,
