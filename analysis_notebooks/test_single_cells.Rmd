...
cell_ratio_other_plots_filepath <- file.path(plots_dir, "single-cell_cell_ratio_other.pdf")
```
...
for(x in ids) {
  ...
  cell_prop_plot_objects[[x]] <- bind_cols(labels_df, umap_df) %>%
    ...
    facet_grid(cols = vars(cluster, max_score),
               space = "free_x",
               scales = "free_x") +
  ...
  cell_prop_other_plot_objects[[x]] <- bind_cols(labels_df, umap_df) %>%
    arrange(confidence) %>%
    group_by(cluster, max_score) %>%
    mutate(id = row_number()) %>%
    pivot_longer(cols = c("max", "other"),
                 names_to = "subgroup",
                 values_to = "score") %>%
    ggplot(aes(x = id,
               y = score,
  ...
  cell_ratio_plot_objects[[x]] <- bind_cols(labels_df, umap_df) %>%
    ...
    geom_point(position = position_jitterdodge(height = 0, width = 0.1),
               shape = 16) +
    scale_color_viridis_d() +
    labs(title = x,
         x = "Cluster",
         y = "Score Ratio: max/other",
         fill = "Predicted\nSubgroup") +
    theme_bw()
...
ggplot2::ggsave(filename = cell_prop_other_plots_filepath,
                plot = patchwork::wrap_plots(cell_prop_other_plot_objects[1:3], ncol = 1),
                width = 7.5, height = 10)

ggplot2::ggsave(filename = cell_prop_other_plots_normal_filepath,
                plot = patchwork::wrap_plots(cell_prop_other_plot_objects[4:7], ncol = 1),
                width = 7.5, height = 10)

ggplot2::ggsave(filename = cell_ratio_plots_filepath,
                plot = patchwork::wrap_plots(cell_ratio_plot_objects[1:3], ncol = 1),
                width = 7.5, height = 10)

ggplot2::ggsave(filename = cell_ratio_plots_normal_filepath,
                plot = patchwork::wrap_plots(cell_ratio_plot_objects[4:7], ncol = 1),
                width = 7.5, height = 10)
