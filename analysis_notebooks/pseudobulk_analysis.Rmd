...
mp_smartseq_perf <- calculate_confusion_matrix(
  predicted_labels = mp_smartseq_df$predicted_subgroup,
...
mp_10x_perf <- calculate_confusion_matrix(
  predicted_labels = mp_10x_df$predicted_subgroup,
...
```{r}
pseudobulk_plot_df <- pseudobulk_plot_df |>
...
rule_based_df_list <- list(
  rf_smartseq = pseudobulk_plot_df |>
    dplyr::filter(study == "GSE119926",
                  model_type == "RF (w)"),
  
  rf_10x = pseudobulk_plot_df |>
    dplyr::filter(study == "GSE155446",
                  model_type == "RF (w)"),
  
  ktsp_smartseq = pseudobulk_plot_df |>
    dplyr::filter(study == "GSE119926",
                  model_type == "kTSP (w)"),
  
  ktsp_10x = pseudobulk_plot_df |>
    dplyr::filter(study == "GSE155446",
                  model_type == "kTSP (w)")
)
```

## Confusion matrices for all model-dataset pairs

```{r}
cm_list <- rule_based_df_list |>
  purrr::map(
    \(model_experiment_pair) { calculate_confusion_matrix(
        predicted_labels = model_experiment_pair$predicted_subgroup,
        true_labels = model_experiment_pair$subgroup,
        labels = mb_subgroups
    ) }
  )

cm_list[["mp_smartseq"]] <- mp_smartseq_perf
cm_list[["mp_10x"]] <- mp_10x_perf
```

```{r}
cm_list |> 
  purrr::imap(
    \(confusion_matrix, model_experiment_pair) data.frame(
      "name" = model_experiment_pair,
...
```{r}
expand.grid(c("GSE119926", "GSE155446"), 
            c("RF (w)", "kTSP (w)", "medulloPackage")) |>
  dplyr::rename(study = Var1,
                model_type = Var2)
```
