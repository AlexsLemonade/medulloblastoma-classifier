...
mp_smartseq_perf <- caret::confusionMatrix(
  data = factor(mp_smartseq_df$predicted_subgroup,
                levels = mb_subgroups),
  reference = factor(mp_smartseq_df$subgroup,
                     levels = mb_subgroups)
)

# Make 10x dataset confusion matrix
mp_10x_perf <- caret::confusionMatrix(
  data = factor(mp_10x_df$predicted_subgroup,
                levels = mb_subgroups),
  reference = factor(mp_10x_df$subgroup,
                     levels = mb_subgroups)
)
```
...
## Get the predicted label for the rule-based models
...
```{r}
rf_smartseq_df <- pseudobulk_plot_df |>
  dplyr::filter(study == "GSE119926",
                model_type == "RF (w)") |>
  dplyr::rowwise() |> 
  dplyr::mutate(
...
rf_10x_df <- pseudobulk_plot_df |>
  ...
  dplyr::ungroup()
ktsp_smartseq_df <- pseudobulk_plot_df |>
  dplyr::filter(study == "GSE119926",
                model_type == "kTSP (w)") |> 
  dplyr::rowwise() |> 
  dplyr::mutate(
...
ktsp_10x_df <- pseudobulk_plot_df |>
  dplyr::filter(study == "GSE155446",
                model_type == "kTSP (w)")|> 
  dplyr::rowwise() |> 
  dplyr::mutate(
...
rf_smartseq_perf <- caret::confusionMatrix(
  data = factor(rf_smartseq_df$predicted_subgroup,
                levels = mb_subgroups),
  reference = factor(rf_smartseq_df$subgroup,
                     levels = mb_subgroups)
)

rf_10x_perf <- caret::confusionMatrix(
  data = factor(rf_10x_df$predicted_subgroup,
                levels = mb_subgroups),
  reference = factor(rf_10x_df$subgroup,
                     levels = mb_subgroups)
)

ktsp_smartseq_perf <- caret::confusionMatrix(
  data = factor(ktsp_smartseq_df$predicted_subgroup,
                levels = mb_subgroups),
  reference = factor(ktsp_smartseq_df$subgroup,
                     levels = mb_subgroups)
)

ktsp_10x_perf <- caret::confusionMatrix(
  data = factor(ktsp_10x_df$predicted_subgroup,
                levels = mb_subgroups),
  reference = factor(ktsp_10x_df$subgroup,
                     levels = mb_subgroups)
)
```
