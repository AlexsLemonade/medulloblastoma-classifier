...
ktsp_model <- baseline_models[[official_model]]$ktsp_weighted$classifier
...
We can see that cells in the 10X experiment tend to be classified as G3 even when the sample's subgroup label is not G3, and this is particularly pronounced in the RF model.
...
Cells that have predicted labels that are different from the sample's true subgroup labels appear to tend to have fewer total genes detected.
...
This suggests that the cells getting incorrectly labeled as G3 tend to have TPM = 0 for many of the genes in the RF model in the 10X dataset.
...
Thus, it's more helpful to look at the percentage of genes with TPM = 0 on a per subgroup basis.
...
ktsp_informative_genes_df <- ktsp_df |>
  ...
  dplyr::group_by(sample_accession,
                  cell_index,
                  gene_subgroup) |>
  # Number genes TPM == 0 / number of genes
  dplyr::summarize(percent_zeros = sum(TPM == 0) / dplyr::n()) |>
...
```{r}
corrected_classified_with_all_zeros_df <- ktsp_informative_genes_df |>
  dplyr::filter(correct_classification, percent_zeros == 1)

corrected_classified_with_all_zeros_df |>
...
Because we set `weighted_votes = TRUE`, this is probably equivalent to one of the six SHH rules being true in every case.

```{r}
# Get wide version of TPM, only for relevant genes, i.e., in the kTSP models
exprs_shh_zeros <- corrected_classified_with_all_zeros_df |>
  dplyr::filter(predicted_labels == "SHH") |>
  dplyr::select(sample_accession, cell_index) |>
  dplyr::left_join(dplyr::filter(expression_df, 
                                 ...
                                 model_type == "kTSP (w)"),
                   by = c("sample_accession", "cell_index")) |>
  dplyr::select(sample_accession, cell_index, gene, TPM) |>
