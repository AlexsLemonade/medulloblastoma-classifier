...
ktsp_informative_genes_df <- ktsp_df |> 
  ...
  # Only keep the rows with the gene subgroup relevant to the prediction 
  # Or if incorrectly classified, the true subgroup
  dplyr::filter(gene_subgroup == predicted_labels | gene_subgroup == subgroup)
```

```{r}
ktsp_informative_genes_df |>
  ggplot(aes(x = percent_zeros,
             fill = correct_classification,
             group = correct_classification)) +
  geom_density(alpha = 0.2) +
  facet_grid(rows = vars(gene_subgroup), cols = vars(study)) +
  theme_bw()
```
