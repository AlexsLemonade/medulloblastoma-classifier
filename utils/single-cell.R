...
test_rf_single_cells <- function(sample_acc,
                                 sce_filepath,
                                 metadata_df,
                                 labels,
                                 classifier,
  ...
  stopifnot(
    "Classifier type not rule_based_RandomForest" = (class(classifier) == "rule_based_RandomForest")
  )
  ...
  # If there are any cells retained, predict as normal
...
test_ktsp_single_cells <- function(sample_acc,
                                   sce_filepath,
                                   metadata_df,
                                   labels,
                                   classifier,
  ...
  filtering_type <- tolower(filtering_type)
  stopifnot(
    "Classifier type not OnevsrestScheme_TSP" = (class(classifier) == "OnevsrestScheme_TSP"),
  ...
  # If at least one cell is retained, test as normal
  if (ncol(genex_df_this_sample) > 0) {
  ...
  } else {  # Otherwise, initialize empty data frames
  ...
  # If there are some cells that will not be classified
  if (length(unclassified_cell_ids) > 0) {
